...
function App(props) {
  ...
  // async function destroyCarLocal() {
  //  if (localCar.length === 0) {
  //   try {await AsyncStorage.removeItem('carrito');}
 ...
 let totales = 0
  ...
  async function goToBack() {
   ...
   //await navigation.goBack()
    // navigation.navigate("Shop2", { randomCode: Math.random() })
  }
...
<Head name_user={userDetails.nombres} />
        ...
        <View style={styles.content_head}>
          ...
          <Text style={{ fontSize: 20, fontWeight: "bold", top:25 }}>Mi Pedido</Text>
        ...
        <View style={styles.content_cart}>
          {/* <Text>porct. {porcentaje}%</Text>
          <Text>COP.{PriceDescount}</Text> */}
          {
            ...
            <View style={{ borderColor: colorDark, borderWidth: 2, padding: 10, borderRadius: 20, borderStyle: 'dashed', textAlign: "center", }}>
              <Text style={{ textAlign: "center", width: "100%", color: colorDark, textTransform: "uppercase", fontSize: 16, }}>carrito de compras vacio</Text>
            </View>
          }
          {
            Load == true &&
              ...
              return (
                <View style={styles.content_cart_product_item} key={key}>
                  ...
                  <View
                    ...
                    <View style={
                      // styles.content_cart_product_item_description
                      {
                        // width: "100%",
                        // paddingTop: 10,
                        // paddingLeft: 10,
        ...
        <View style={{ height: 250,  overflow: "hidden" }}></View>
      ...
      <View style={styles.wraptotal}>
        <View style={styles.options}>
          <TouchableOpacity onPress={() => setOptions()} style={{ width: "100%", flexDirection: "row", borderBottomColor: "#777", borderBottomWidth: 0.3 }}>
            <Text>Opciones </Text>
            <Icon
              style={{ marginLeft: "70%", top: -5, }}
              name={ViewOptions == true ? 'arrow-ios-upward-outline' : 'arrow-ios-downward-outline'}
              width={25} height={25} fill='#777' />
          </TouchableOpacity>
          {/* <View style={[styles.row, { height: ViewOptions == true ? 40 : 0 }]}>
             <Text style={styles.pregunta}>Adicionar envio?</Text>
             <CheckBox value={chekEnvio} onValueChange={setChekEnvio} style={styles.checkbox} />
             <Text style={styles.respuesta}>COP. {priceEnvio}</Text>
           </View>  */}
          {
            Cupon == false &&
          ...
          {/* <CheckBox value={Cupon} style={styles.checkbox} disabled={true} />
            {isNaN(porcentaje) &&
              <Text style={styles.pregunta}>{porcentaje}</Text>
            }
            {
              porcentaje > 0 &&
              <Text style={styles.pregunta}>{porcentaje}%</Text>
        ...
        <View style={styles.total}>
          ...
          {
            localCar.length != 0 &&
            <TouchableOpacity onPress={() => GoToPay("AppCompras")} style={styles.BtnPrimary}>
              <Icon name='credit-card-outline' width={35} height={35} fill='#FFF' />
            </TouchableOpacity>
          }
          <Text style={{ marginLeft: 5, fontSize: 20, fontWeight: "bold", justifyContent: "center" }}>
