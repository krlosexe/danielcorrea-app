...
function PostsDetailScreen(props) {
  ...
  React.useEffect(() => {
    setLoad(true)
    console.log(base_url(postsServer, `api/get/posts/17`))
    axios.get(base_url(postsServer, `api/get/posts/17`)).then((response) => {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#F3F3F3" }}>
      ...
      <ScrollView contentContainerStyle={{
        alignContent: "center", alignItems: "center", flexGrow: 1, paddingTop: 20, 
      }}>
        {
          Load &&
          <Loading color={primaryColor} />
...
function Card1(props) {
  ...
  function Wrapper() {
    console.log(showMore ? "show1" : "hidden1");
  ...
  return (
    <View style={{ backgroundColor: "white", top: 20, marginBottom: 30, padding: 10, width: '90%', borderRadius: 20 }}>
      <View style={{ width: '100%', padding: 20 }}>
        ...
        <Text style={{ height: showMore ? null : 60, padding: 5, flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-around', }}>
          {props.item.post}
        </Text>
        <TouchableOpacity onPress={() => Wrapper()} style={{ alignSelf: "flex-end" }}><Text style={{ color: primaryColor, fontSize: 14 }}>{showMore ? "Ver menos..." : "Ver más..."}</Text></TouchableOpacity>
      </View>
...
function Card2(props) {
  const [showMore, setShowMore] = useState(false);
  function Wrapper() {
    console.log(showMore ? "show2" : "hidden2");
    setShowMore(!showMore);
  }
  return (
    <View style={{ backgroundColor: "white", top: 20, marginBottom: 30, padding: 10, width: '90%', borderRadius: 20 }}>
      <View style={{ width: '100%', padding: 20, }}>
        ...
        <Text style={{ height: showMore ? null : 48, padding: 5, flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-around', }}>
          {props.item.post}
        </Text>
        <TouchableOpacity onPress={() => Wrapper()} style={{ alignSelf: "flex-end" }}><Text style={{ color: primaryColor, fontSize: 14 }}>{showMore ? "Ver menos..." : "Ver más..."}</Text></TouchableOpacity>
      </View>
      ...
      <View style={{ paddingVertical: 5, flexDirection: 'row', justifyContent: "space-around", borderTopColor: "#c3c3c3", borderTopWidth: 0.5 }}>
        <TouchableOpacity style={styles.buttonContainer} onPress={() => props.share(props.item)}>
          <Icon style={styles.buttonIcon} name={'share-outline'} width={20} height={20} fill={primaryColor} />
          <Text style={styles.buttonText}>2 Compartir</Text>
        </TouchableOpacity>
